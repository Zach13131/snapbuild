...
export function Security() {
  return (
    <section id="features" className="flex flex-col gap-8 px-4 py-8 md:px-5 lg:px-10">
      <Reveal className="flex min-h-[220px] flex-col items-center justify-center rounded-[24px] bg-white px-6 py-16 text-center">
        <h2 className="text-[28px] leading-[1.2] font-semibold tracking-[-0.03em] md:text-[40px] lg:text-[48px] lg:leading-[1.33]">
      ...
      <div className="grid grid-cols-3 gap-2 md:gap-6 lg:gap-8">
        {securityCards.map((card, index) => (
          <Reveal key={card.title} as="article" delay={index * 50} className="card-lift flex min-w-0 flex-col gap-2 md:gap-4">
            <picture>
              <source media="(max-width: 767px)" srcSet={asset(card.imageMobile)} />
              <img
                src={asset(card.image)}
                alt=""
                className="aspect-square w-full rounded-[6px] object-cover md:rounded-xl lg:h-[min(280px,22vw)] lg:aspect-auto lg:rounded-[20px]"
              />
            </picture>
